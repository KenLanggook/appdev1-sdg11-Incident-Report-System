...
export class IncidentMapComponent implements OnInit, AfterViewInit {
  ...
  // University of Baguio coordinates (Main Campus - Gen. Luna Road, Baguio City)
  private readonly UB_LATITUDE = 16.4166;
  private readonly UB_LONGITUDE = 120.5931;
  
  // University of Baguio campus boundaries
...
>>>>>>> 54d57371d7e07b666eb9d8040b9700021cbd302b
  ...
  private initializeMap() {
    ...
    if (mapElement) {
      this.map = L.map('map', {
        ...
        zoom: 18,
        maxBounds: [
          this.CAMPUS_BOUNDS.southwest,
          this.CAMPUS_BOUNDS.northeast
        ],
      ...
      // Add boundary restriction message
      this.addBoundaryRestriction();
    }
  }

  private addCampusBoundary() {
    // University of Baguio campus boundary coordinates (Gen. Luna Road campus)
    const campusCoordinates = [
      [16.4140, 120.5910], // Southwest corner
      [16.4140, 120.5950], // Southeast corner
      [16.4190, 120.5950], // Northeast corner
      [16.4190, 120.5910], // Northwest corner
      [16.4140, 120.5910]  // Back to start
    ];
    ...
    // Add boundary label
    const centerPoint = [
      (this.CAMPUS_BOUNDS.southwest[0] + this.CAMPUS_BOUNDS.northeast[0]) / 2,
      (this.CAMPUS_BOUNDS.southwest[1] + this.CAMPUS_BOUNDS.northeast[1]) / 2
    ];
  ...
  private addBoundaryRestriction() {
    // Add a control to show boundary information
    const boundaryControl = L.control({ position: 'topright' });
    boundaryControl.onAdd = () => {
      ...
      div.innerHTML = `
        ...
        ">
          CAMPUS BOUNDARY<br>
          <small>Incidents can only be reported<br>within University of Baguio</small>
        </div>
      `;
      return div;
    };
    boundaryControl.addTo(this.map);
  }
